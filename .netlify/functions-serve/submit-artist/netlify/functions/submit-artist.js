...
function mapFields(input) {
  ...
  const fullName = v(input.fullName) || v(input.name) || v(input.full_name) || v(input.applicant_name);
  ...
  const igHandle = v(input.ig_handle) || v(input.instagram) || v(input.instagram_handle);
  const portfolioLink = v(input.portfolio) || v(input.portfolio_link) || v(input.website) || v(input.url) || v(input.link);
  const proud = v(input.q_proud) || v(input.proud) || v(input.q_vision) || v(input.question_proud);
  const commitment = v(input.q_commitment) || v(input.commitment) || v(input.long_term);
  const agreeSanitationRaw = input.agree_sanitation ?? input.agreeSanitation ?? input.sanitation ?? input.agree;
  const agreeSanitation = ["true", "on", "yes", "1"].includes(
    String(agreeSanitationRaw).toLowerCase()
  );
  const videoUrl = v(input.video_url) || v(input.video) || v(input.resume_link) || v(input.video_intro);
  return {
    fullName,
    phone,
    email,
...
//# sourceMappingURL=submit-artist.js.map
